 ...
 * 描述：用户列表
 */
ctrlModule.controller("RoleListCtrl",["$scope","$rootScope","$state","$http","$cookieStore",
    function($scope,$rootScope,$state,$http,$cookieStore){
        $scope.current_page = 1; 
        $scope.getData = function(){
            $scope.loading = true;
            var url = $rootScope.roleSearchUrl;
            var ajax = new ajaxClass($http,url,"GET");
            ajax.params = {"PageIndex":$scope.current_page,"PageNumber":"10","RoleName":""};
            ...
            ajax.successCallback = function(res){
                ...
                if(res.status == "200"){
                    ...
                    var totalItems = Math.ceil($scope.roleList.TotalItems / 10)
