 ...
 * 描述：用户列表
 */
ctrlModule.controller("DeviceListCtrl",["$scope","$rootScope","$state","$http","$cookieStore",
    function($scope,$rootScope,$state,$http,$cookieStore){
        $scope.current_page = 1; 
        $scope.getData = function(){
            $scope.loading = true;
            var url = $rootScope.deviceSearchUrl;
            ...
            ajax.successCallback = function(res){
                ...
                if(res.status == "200"){
                    ...
                    var totalItems = Math.ceil($scope.deviceList.TotalItems / 10)
